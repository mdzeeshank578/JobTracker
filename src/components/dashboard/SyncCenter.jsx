...
export default function SyncCenter({ jobs }) {
  ...
  const [customSyncEmail, setCustomSyncEmail] = useState('');

  // Connect provider redirection
  const handleConnectProvider = (provider) => {
    if (provider === 'google') {
      setShowSyncGoogleModal(true);
    } else {
      window.location.href = `${backendUrl}/api/auth/connect/${provider}?userId=${userId}`;
    }
  };

  const handleExecuteGoogleConnect = (chosenEmail) => {
    const finalEmail = chosenEmail || customSyncEmail || (currentUser ? currentUser.email : '');
    if (!finalEmail || !finalEmail.includes('@')) {
      alert("Please specify a valid Google email address to connect.");
      return;
    }
    setShowSyncGoogleModal(false);
    window.location.href = `${backendUrl}/api/auth/connect/google?userId=${userId}&email=${encodeURIComponent(finalEmail)}`;
  };
  ...
  return (
    <div className="sync-center-container">
      ...
      {/* Google Account Connection Picker Modal */}
      {showSyncGoogleModal && (
        <div style={{
          position: 'fixed',
          inset: 0,
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 6px 0', fontSize: '1.2rem', color: '#0f172a' }}>Connect Google / Gmail Account</h3>
            <p style={{ margin: '0 0 20px 0', fontSize: '0.86rem', color: '#64748b' }}>Select or enter the Google email account you want to connect for automated tracking:</p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px', textAlign: 'left' }}>
              {currentUser?.email && (
                <button 
                  type="button" 
                  onClick={() => handleExecuteGoogleConnect(currentUser.email)}
                  style={{
                    padding: '10px 14px',
                    borderRadius: '8px',
            ...
            <div style={{ fontSize: '0.82rem', color: '#94a3b8', marginBottom: '10px' }}>— Or connect custom Gmail account —</div>

            <input 
              type="email" 
              placeholder="name@gmail.com" 
              value={customSyncEmail}
              onChange={(e) => setCustomSyncEmail(e.target.value)}
              style={{
                width: '100%',
                padding: '10px 14px',
                borderRadius: '8px',
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button 
                ...
                style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid #cbd5e1', background: 'white', cursor: 'pointer', fontSize: '0.88rem' }}
              >
                Cancel
              </button>
              <button 
                type="button" 
                onClick={() => handleExecuteGoogleConnect(customSyncEmail)}
                style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#2563eb', color: 'white', fontWeight: 600, cursor: 'pointer', fontSize: '0.88rem' }}
              >
                Connect Account →
              </button>
            </div>
